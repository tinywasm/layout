...
> **es** el arnés*.
>
> **Publicado:** `github.com/tinywasm/widget v0.3.0`, `github.com/tinywasm/css v0.3.0` y
> `github.com/tinywasm/form v0.3.1` ya están etiquetados.
>
> **Pendientes y su orden — ver §7.** Cada repo se migra **de una sola vez, no por etapas
> escalonadas**: no hay canario ni periodo de coexistencia dentro de un repo. Pero entre repos
> **sí hay orden**, y `tinywasm/ssr` es el gate de los otros tres: hasta que publique, renombrar
> `Style()` en `components` rompe la extracción de assets. Cada plan lleva su propio chequeo de
> gate ejecutable en su §0.
...
C --> E[css.go<br/>RenderCSS → Stylesheet · solo !wasm]
...
| CSS nunca emitido por nombrar la función `GenerateCSS` | Diagnóstico ruidoso en `ssr`: un `css.go` sin `RootCSS()` ni `RenderCSS()` aborta la extracción ([ssr/docs/PLAN.md](https://github.com/tinywasm/ssr/blob/main/docs/PLAN.md) §4) |
...
|---|---|---|---|---|
| `tinywasm/widget` | **Nueva.** Contrato visual + `widget/style` | [PLAN_WIDGET](PLAN_WIDGET.md) | **v0.3.0** | ✅ Publicado |
...
Los cuatro pendientes **sí están escalonados**, y el orden no es negociable:

```mermaid
flowchart LR
    A["ssr<br/>borra la rama Style()<br/><b>GATE</b>"] --> B["components<br/>Style() → RenderCSS()"]
    A --> C["widget<br/>borra Styler<br/>+ escala Motion"]
    B --> D["layout<br/>crudview · platformd · rightpanel"]
    C --> D
```

- **`ssr` es el gate de todo.** Su extractor publicado (v0.0.24) aborta con error si un paquete
  importa `widget/style` sin declarar `Style()` — que es exactamente el estado que produce el
  plan de `components`. Renombrar antes de que `ssr` publique rompe la extracción de assets de
  cualquier app.
- **`components` y `widget` van en paralelo** una vez `ssr` está publicado. No se bloquean entre
  sí: `components` importa `widget/style` para construir, pero nunca la interfaz `Styler`.
- **`layout` es el último** y necesita las dos publicaciones anteriores: el `components` con
  `RenderCSS()` y el `widget` con `Animate(Motion)` (que `platformd` consume).

Cada uno se ejecuta de una sola vez, sin canario ni periodo de coexistencia.

---

## 8. Migración de `tinywasm/layout` — un solo cambio, no por etapas

`tinywasm/widget` (v0.3.0) y `tinywasm/css` (v0.3.0) ya están publicados y probados con su test
...
3. Toda `var(--…)` referenciada existe en el catálogo de `css` v0.3.0.
